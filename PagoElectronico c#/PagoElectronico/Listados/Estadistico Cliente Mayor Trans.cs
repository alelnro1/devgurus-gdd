...
{
    public partial class Estadistico_Cliente_Mayor_Trans : Form
    {
        private EstadisticasDAO estadisticasDAO;

        public Estadistico_Cliente_Mayor_Trans(string anio, int trimestre)
        {
            InitializeComponent();
        ...
        {
            Info_estadistica.Text = "Clientes con mas transferencias entre cuentas propias en el año " +
            anio + " y periodo " + dameElNombreDeTrimestre(trimestre) + ".";
            string procedimiento = ConstantesBD.proc_listar_Mayores_Trans;
            ...
            while (lector.Read())
            {
                columnas[0] = lector["Cuenta_Cliente"];
                columnas[1] = lector["Cliente_Nombre"];
                columnas[2] = lector["Cliente_Apellido"];
                columnas[3] = lector["Cantidad_De_Transferencias"];
